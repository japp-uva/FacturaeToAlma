...
class AlmaPOLineLookup:
    REQUIRED_COLUMNS = ["PO Line", "Reporting Code", "Secondary Reporting Code", "Tertiary Reporting Code", "Start subs date", "Title", "ISSN", "ISBN"]
...
class ValidationReportExporter:
    ...
    def save(self, issues):
        ...
        ws = wb.active
        ws.title = "Validacion"
        ws.append(["Tipo aviso", "Factura", "Linea factura", "ISBN XSIG/XML", "ISSN XSIG/XML", "Titulo", "PO Lines candidatas", "Mensaje"])
        for issue in issues:
            ws.append([sanitize_excel_value(v) for v in [issue.issue_type, issue.invoice_number, issue.invoice_line_number, issue.isbn, issue.issn, issue.title, "; ".join(issue.candidates), issue.message]])
